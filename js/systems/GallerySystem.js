...
export class GallerySystem {
    ...
    initBossData() {
        return [
            ...
            { id: 'bsd_swimsuit', level: 6, name: '波塞冬·特别服装', title: 'Poseidon Special', lockedName: '波塞冬·???', lockedTitle: '???', isMutated: false, image: 'bsd swim suit.PNG', wideImage: true, lockedImage: 'bsd locked.PNG', unlockType: 'all_except_lv7', unlockCount: 2, unlockHint: '除阿尔忒弥斯外Boss每个击杀2次解锁' },
    ...
    isUnlocked(bossId) {
        ...
        if (boss.unlockType === 'all_mutated') {
            // 全部异化Boss都击杀达到指定次数
            const mutatedBosses = this.bossData.filter(b => b.isMutated);
            return mutatedBosses.every(b => this.getKillCount(b.id) >= boss.unlockCount);
        }
